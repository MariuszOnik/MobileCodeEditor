...
</html>`,t.appendChild(n),I=n}function L(e){I&&=(I.remove(),null),e.querySelectorAll(`iframe`).forEach(e=>e.remove())}function Ce(e){return new Worker(`/MobileCodeEditor/assets/compiler.worker-C6_MBn_9.js`,{type:`module`,name:e?.name})}var R=new Ce,z=new Map;R.onmessage=e=>{let{id:t,code:n,errors:r}=e.data;z.get(t)?.({code:n,errors:r}),z.delete(t)};function we(e,t){return new Promise(n=>{let r=Math.random().toString(36).slice(2);z.set(r,n),R.postMessage({id:r,files:e,entryPoint:t})})}var Te=document.getElementById(`app`),Ee=document.getElementById(`sidebar`),B=document.getElementById(`editor-container`),V=document.getElementById(`run-container`),H=document.getElementById(`status`),U=document.getElementById(`tab-editor`),W=document.getElementById(`tab-run`),G=document.getElementById(`tab-files`),De=document.getElementById(`btn-run`),Oe=document.getElementById(`btn-stop`),ke=document.getElementById(`btn-save`),Ae=document.getElementById(`btn-open-dir`),je=document.getElementById(`current-filename`),K=null,q=null,J=null,Y=!1;async function Me(){await Pe(),J=new be(document.getElementById(`file-tree`)),await J.refresh();let e=await k();e.length&&X(e[0]),N(F.FILE_OPEN,e=>X(e)),N(F.FILE_CREATE,()=>J?.refresh()),N(F.FILE_DELETE,e=>{q===e&&(q=null)}),U.addEventListener(`click`,()=>Q(`editor`)),W.addEventListener(`click`,()=>Q(`run`)),G.addEventListener(`click`,()=>Ie()),De.addEventListener(`click`,Fe),Oe.addEventListener(`click`,()=>{L(V),$(`Zatrzymano`,`info`)}),ke.addEventListener(`click`,Z),Ae.addEventListener(`click`,Le),document.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.key===`s`&&(e.preventDefault(),Z())}),window.visualViewport&&(window.visualViewport.addEventListener(`resize`,Ne),window.visualViewport.addEventListener(`scroll`,Ne)),Q(`editor`)}function Ne(){let e=window.visualViewport,t=document.getElementById(`app`);t.style.height=`${e.height}px`,t.style.transform=`translateY(${e.offsetTop}px)`,requestAnimationFrame(()=>K?.layout())}async function Pe(){(await k()).length>0||await D(`main.ts`,`import Phaser from 'phaser'
