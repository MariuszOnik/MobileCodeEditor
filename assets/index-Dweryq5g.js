...
`,d=i.typescript;if(d?.typescriptDefaults){let e={target:d.ScriptTarget?.ES2022??99,module:d.ModuleKind?.ESNext??99,allowNonTsExtensions:!0,allowSyntheticDefaultImports:!0,esModuleInterop:!0,checkJs:!0};d.typescriptDefaults.addExtraLib(u,`ts:engine-defs.d.ts`),d.typescriptDefaults.setCompilerOptions(e),d.javascriptDefaults.addExtraLib(u,`ts:engine-defs.d.ts`),d.javascriptDefaults.setCompilerOptions(e)}function ee(e,t,r=`typescript`){return n.create(e,{value:t,language:r,theme:`vs-dark`,fontSize:14,minimap:{enabled:!1},lineNumbers:`on`,scrollBeyondLastLine:!1,automaticLayout:!0,tabSize:2,wordWrap:`off`,scrollBeyondLastColumn:8,scrollbar:{horizontal:`auto`,vertical:`auto`,useShadows:!1},padding:{top:8},lineDecorationsWidth:8,renderLineHighlight:`line`})}function te(e){return e.endsWith(`.ts`)?`typescript`:e.endsWith(`.html`)||e.endsWith(`.htm`)?`html`:e.endsWith(`.css`)?`css`:e.endsWith(`.json`)?`json`:`javascript`}var f=(e,t)=>t.some(t=>e instanceof t),ne,re;function ie(){return ne||=[IDBDatabase,IDBObjectStore,IDBIndex,IDBCursor,IDBTransaction]}function ae(){return re||=[IDBCursor.prototype.advance,IDBCursor.prototype.continue,IDBCursor.prototype.continuePrimaryKey]}var p=new WeakMap,m=new WeakMap,h=new WeakMap;function oe(e){let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`success`,i),e.removeEventListener(`error`,a)},i=()=>{t(v(e.result)),r()},a=()=>{n(e.error),r()};e.addEventListener(`success`,i),e.addEventListener(`error`,a)});return h.set(t,e),t}function se(e){if(p.has(e))return;let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`complete`,i),e.removeEventListener(`error`,a),e.removeEventListener(`abort`,a)},i=()=>{t(),r()},a=()=>{n(e.error||new DOMException(`AbortError`,`AbortError`)),r()};e.addEventListener(`complete`,i),e.addEventListener(`error`,a),e.addEventListener(`abort`,a)});p.set(e,t)}var g={get(e,t,n){if(e instanceof IDBTransaction){if(t===`done`)return p.get(e);if(t===`store`)return n.objectStoreNames[1]?void 0:n.objectStore(n.objectStoreNames[0])}return v(e[t])},set(e,t,n){return e[t]=n,!0},has(e,t){return e instanceof IDBTransaction&&(t===`done`||t===`store`)?!0:t in e}};function _(e){g=e(g)}function ce(e){return ae().includes(e)?function(...t){return e.apply(y(this),t),v(this.request)}:function(...t){return v(e.apply(y(this),t))}}function le(e){return typeof e==`function`?ce(e):(e instanceof IDBTransaction&&se(e),f(e,ie())?new Proxy(e,g):e)}function v(e){if(e instanceof IDBRequest)return oe(e);if(m.has(e))return m.get(e);let t=le(e);return t!==e&&(m.set(e,t),h.set(t,e)),t}var y=e=>h.get(e);function ue(e,t,{blocked:n,upgrade:r,blocking:i,terminated:a}={}){let o=indexedDB.open(e,t),s=v(o);return r&&o.addEventListener(`upgradeneeded`,e=>{r(v(o.result),e.oldVersion,e.newVersion,v(o.transaction),e)}),n&&o.addEventListener(`blocked`,e=>n(e.oldVersion,e.newVersion,e)),s.then(e=>{a&&e.addEventListener(`close`,()=>a()),i&&e.addEventListener(`versionchange`,e=>i(e.oldVersion,e.newVersion,e))}).catch(()=>{}),s}var de=[`get`,`getKey`,`getAll`,`getAllKeys`,`count`],fe=[`put`,`add`,`delete`,`clear`],b=new Map;function pe(e,t){if(!(e instanceof IDBDatabase&&!(t in e)&&typeof t==`string`))return;if(b.get(t))return b.get(t);let n=t.replace(/FromIndex$/,``),r=t!==n,i=fe.includes(n);if(!(n in(r?IDBIndex:IDBObjectStore).prototype)||!(i||de.includes(n)))return;let a=async function(e,...t){let a=this.transaction(e,i?`readwrite`:`readonly`),o=a.store;return r&&(o=o.index(t.shift())),(await Promise.all([o[n](...t),i&&a.done]))[0]};return b.set(t,a),a}_(e=>({...e,get:(t,n,r)=>pe(t,n)||e.get(t,n,r),has:(t,n)=>!!pe(t,n)||e.has(t,n)}));var me=[`continue`,`continuePrimaryKey`,`advance`],he={},x=new WeakMap,ge=new WeakMap,_e={get(e,t){if(!me.includes(t))return e[t];let n=he[t];return n||=he[t]=function(...e){x.set(this,ge.get(this)[t](...e))},n}};async function*ve(...e){let t=this;if(t instanceof IDBCursor||(t=await t.openCursor(...e)),!t)return;t=t;let n=new Proxy(t,_e);for(ge.set(n,t),h.set(n,y(t));t;)yield n,t=await(x.get(n)||t.continue()),x.delete(n)}function ye(e,t){return t===Symbol.asyncIterator&&f(e,[IDBIndex,IDBObjectStore,IDBCursor])||t===`iterate`&&f(e,[IDBIndex,IDBObjectStore])}_(e=>({...e,get(t,n,r){return ye(t,n)?ve:e.get(t,n,r)},has(t,n){return ye(t,n)||e.has(t,n)}}));var be=e({deleteFile:()=>E,hasNativeFs:()=>k,listFiles:()=>D,openNativeFolder:()=>Ce,readFile:()=>w,writeFile:()=>T}),xe=`mce-fs`,S=`files`,Se=null;async function C(){return Se||=await ue(xe,1,{upgrade(e){e.createObjectStore(S,{keyPath:`path`})}}),Se}async function w(e){return(await(await C()).get(S,e))?.content??null}async function T(e,t){await(await C()).put(S,{path:e,content:t,updatedAt:Date.now()})}async function E(e){await(await C()).delete(S,e)}async function D(){return(await(await C()).getAll(S)).map(e=>e.path).sort()}var O=null;function k(){return`showDirectoryPicker`in window}async function Ce(){if(!k())throw Error(`File System Access API not supported`);O=await window.showDirectoryPicker({mode:`readwrite`}),await we()}async function we(){if(O)for await(let[e,t]of O.entries())t.kind===`file`&&(e.endsWith(`.ts`)||e.endsWith(`.js`))&&await T(e,await(await t.getFile()).text())}var A=new Map;function j(e,t){return A.has(e)||A.set(e,new Set),A.get(e).add(t),()=>A.get(e)?.delete(t)}function M(e,t){A.get(e)?.forEach(e=>e(t))}var N={FILE_OPEN:`file:open`,FILE_SAVE:`file:save`,FILE_CREATE:`file:create`,FILE_DELETE:`file:delete`,FILE_RENAME:`file:rename`,RUN_START:`run:start`,RUN_STOP:`run:stop`,COMPILE_OK:`compile:ok`,COMPILE_ERR:`compile:error`,PANEL_SWITCH:`panel:switch`},Te=class{el;activeFile=null;collapsed=new Set;constructor(e){this.el=e,this.el.className=`file-tree`,this.render()}async refresh(){let e=Ee(await D());this.renderTree(e)}setActive(e){this.activeFile=e,this.el.querySelectorAll(`.ft-item`).forEach(t=>{t.classList.toggle(`active`,t.getAttribute(`data-path`)===e)})}render(){this.el.innerHTML=``;let e=document.createElement(`div`);e.className=`ft-header`,e.innerHTML=`
    ...
    `,e.querySelector(`.ft-btn-new`).addEventListener(`click`,()=>this.createFile()),e.querySelector(`.ft-btn-folder`).addEventListener(`click`,()=>this.createFolder()),this.el.appendChild(e),this.el.insertAdjacentHTML(`beforeend`,`<div class="ft-body"></div>`),this.refresh()}renderTree(e,t=0){let n=this.el.querySelector(`.ft-body`);t===0&&(n.innerHTML=``),this.renderNodes(n,e,t)}renderNodes(e,t,n){for(let r of t)if(r.type===`folder`){let t=!this.collapsed.has(r.path),i=document.createElement(`div`);i.className=`ft-folder`,i.setAttribute(`data-folder`,r.path),i.innerHTML=`
        ...
        `,i.querySelector(`.ft-folder-header`).addEventListener(`click`,e=>{let t=e.target;t.classList.contains(`ft-del`)||t.classList.contains(`ft-add-here`)||(this.collapsed.has(r.path)?this.collapsed.delete(r.path):this.collapsed.add(r.path),this.refresh())}),i.querySelector(`.ft-add-here`).addEventListener(`click`,e=>{e.stopPropagation(),this.createFile(r.path)}),i.querySelector(`.ft-del`).addEventListener(`click`,async e=>{if(e.stopPropagation(),!await P(`Usuń folder „${r.name}" i wszystkie pliki?`))return;let t=await D();for(let e of t)(e===r.path||e.startsWith(r.path+`/`))&&await E(e);M(N.FILE_DELETE,r.path),await this.refresh()}),e.appendChild(i),t&&r.children?.length&&this.renderNodes(i,r.children,n+1)}else{let t=r.name.split(`.`).pop()??``,i=t===`ts`?`📄`:t===`js`?`📜`:t===`html`||t===`htm`?`🌐`:t===`json`?`📋`:t===`css`?`🎨`:`📝`,a=document.createElement(`div`);a.className=`ft-item`+(r.path===this.activeFile?` active`:``),a.setAttribute(`data-path`,r.path),a.style.paddingLeft=`${8+n*14}px`,a.innerHTML=`
          <span class="ft-icon">${i}</span>
          <span class="ft-name">${r.name}</span>
          <button class="ft-btn ft-del" title="Usuń">✕</button>
        `,a.querySelector(`.ft-name`).addEventListener(`click`,()=>M(N.FILE_OPEN,r.path)),a.querySelector(`.ft-del`).addEventListener(`click`,async e=>{e.stopPropagation(),await P(`Usuń plik „${r.path}"?`)&&(await E(r.path),M(N.FILE_DELETE,r.path),await this.refresh())}),e.appendChild(a)}}async createFile(e){let t=await F(e?`Nowy plik w „${e}":`:`Nowy plik (np. main.ts):`,``);if(!t?.trim())return;let n=e?`${e}/${t.trim()}`:t.trim();await T(n,`// ${n}\n`),M(N.FILE_CREATE,n),await this.refresh(),M(N.FILE_OPEN,n)}async createFolder(){let e=await F(`Nazwa folderu (np. Gra1):`);e?.trim()&&(await T(`${e.trim()}/.gitkeep`,``),await this.refresh())}async copyFileToProject(e,t,n){let r=n??``,i=r?`${r}/${e}`:e;await T(i,t),await this.refresh(),M(N.FILE_OPEN,i)}};function P(e){return new Promise(t=>{let n=I();n.innerHTML=`
    ...
    `,n.querySelector(`.mini-cancel`).addEventListener(`click`,()=>{n.remove(),t(!1)}),n.querySelector(`.mini-ok`).addEventListener(`click`,()=>{n.remove(),t(!0)}),document.body.appendChild(n)})}function F(e,t=``){return new Promise(n=>{let r=I();r.innerHTML=`
      <div class="mini-dialog">
        <div class="mini-msg">${e}</div>
    ...
    `;let i=r.querySelector(`.mini-input`),a=e=>{r.remove(),n(e)};r.querySelector(`.mini-cancel`).addEventListener(`click`,()=>a(null)),r.querySelector(`.mini-ok`).addEventListener(`click`,()=>a(i.value.trim()||null)),i.addEventListener(`keydown`,e=>{e.key===`Enter`&&a(i.value.trim()||null),e.key===`Escape`&&a(null)}),document.body.appendChild(r),requestAnimationFrame(()=>{i.focus(),i.select()})})}function I(){let e=document.createElement(`div`);return e.className=`mini-overlay`,e}function Ee(e){let t=[],n=new Map;for(let r of e){let e=r.split(`/`);if(e.length===1)t.push({type:`file`,name:r,path:r});else{let i=t;for(let t=0;t<e.length-1;t++){let r=e.slice(0,t+1).join(`/`);if(!n.has(r)){let a={type:`folder`,name:e[t],path:r,children:[]};n.set(r,a),i.push(a)}i=n.get(r).children}let a=e[e.length-1];a!==`.gitkeep`&&i.push({type:`file`,name:a,path:r})}}return t}var L=[`png`,`jpg`,`jpeg`,`gif`,`webp`,`svg`],R=[`mp3`,`ogg`,`wav`],z=[...L,...R];function De(e){let t=e.split(`.`).pop()?.toLowerCase()??``;return z.includes(t)}async function Oe(e=``){let t=await D(),n=[];for(let r of t){if(!De(r)||e&&!r.startsWith(e+`/`)&&r!==e)continue;let t=await w(r);if(!t?.startsWith(`data:`))continue;let i=r.split(`/`).pop(),a=i.split(`.`).pop().toLowerCase(),o=L.includes(a)?`image`:R.includes(a)?`audio`:`other`;n.push({key:i.replace(/\.[^.]+$/,``),name:i,path:r,dataUrl:t,type:o})}return n}async function ke(e,t=``){let n=[];for(let r of Array.from(e)){let e=await Ae(r,t);e&&n.push(e)}return n}async function Ae(e,t){let n=e.name.split(`.`).pop()?.toLowerCase()??``;if(!z.includes(n))return null;let r=await je(e),i=t?`${t}/${e.name}`:e.name;await T(i,r);let a=L.includes(n)?`image`:R.includes(n)?`audio`:`other`;return{key:e.name.replace(/\.[^.]+$/,``),name:e.name,path:i,dataUrl:r,type:a}}function je(e){return new Promise((t,n)=>{let r=new FileReader;r.onload=()=>t(r.result),r.onerror=n,r.readAsDataURL(e)})}async function Me(e){await E(e)}var Ne=class{el;folder=``;assets=[];constructor(e){this.el=e,this.el.className=`asset-panel`,this.render()}setFolder(e){this.folder=e,this.refresh()}async refresh(){this.assets=await Oe(this.folder),this.renderGrid()}render(){this.el.innerHTML=``;let e=document.createElement(`div`);e.className=`ft-header`,e.innerHTML=`
    ...
    `,r.querySelector(`.asp-esm`).textContent=t,r.querySelector(`.asp-js`).textContent=n;let i=r.querySelectorAll(`.asp-copy`);i[0].addEventListener(`click`,e=>{e.stopPropagation(),Pe(t,i[0])}),i[1].addEventListener(`click`,e=>{e.stopPropagation(),Pe(n,i[1])}),r.querySelector(`.asp-close`).addEventListener(`click`,e=>{e.stopPropagation(),r.remove()}),document.body.appendChild(r);let a=e=>{r.contains(e.target)||(r.remove(),document.removeEventListener(`click`,a))};setTimeout(()=>document.addEventListener(`click`,a),10)}};async function Pe(e,t){let n=!1;try{await navigator.clipboard.writeText(e),n=!0}catch{let t=document.createElement(`textarea`);t.value=e,t.style.cssText=`position:fixed;top:0;left:0;opacity:0;pointer-events:none`,document.body.appendChild(t),t.focus(),t.select();try{n=document.execCommand(`copy`)}catch{n=!1}t.remove()}t.textContent=n?`✓`:`✗`,setTimeout(()=>{t.textContent=`📋`},1400)}var Fe=class{el;listEl;buildInfoEl;entries=[];visible=!1;maxEntries=200;constructor(e){this.el=e,this.el.className=`console-panel`,this.el.innerHTML=`
...
<\/script>`,Le=JSON.stringify({imports:{phaser:`https://esm.sh/phaser@3.88.2`,"pixi.js":`https://esm.sh/pixi.js@8`,"matter-js":`https://esm.sh/matter-js`}});function Re({code:e,html:t,container:n}){ze(n);let r=document.createElement(`iframe`);r.setAttribute(`sandbox`,`allow-scripts allow-same-origin`),r.style.cssText=`width:100%;height:100%;border:none;background:#000;flex:1`,t?r.srcdoc=t.includes(`<head>`)?t.replace(`<head>`,`<head>`+V):V+t:r.srcdoc=`<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
</html>`,n.appendChild(r),B=r}function ze(e){B&&=(B.remove(),null),e.querySelectorAll(`iframe`).forEach(e=>e.remove())}function Be(e){return new Worker(`/MobileCodeEditor/assets/compiler.worker-CKmcKM0R.js`,{type:`module`,name:e?.name})}var Ve=new Be,H=new Map;Ve.onmessage=e=>{let{id:t,code:n,errors:r}=e.data;H.get(t)?.({code:n,errors:r}),H.delete(t)};function He(e,t){return new Promise(n=>{let r=Math.random().toString(36).slice(2);H.set(r,n),Ve.postMessage({id:r,files:e,entryPoint:t})})}(function(){let e=new Date(`2026-05-27T18:55:12.977Z`),t=e=>e.toString().padStart(2,`0`),n=`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}  ${t(e.getHours())}:${t(e.getMinutes())}`;document.getElementById(`app-build`).textContent=`build  ${n}`})();var Ue=document.getElementById(`app`),We=document.getElementById(`sidebar`),Ge=document.getElementById(`editor-container`),Ke=document.getElementById(`run-container`),U=document.getElementById(`run-iframe-area`),qe=document.getElementById(`status`),Je=document.getElementById(`tab-editor`),Ye=document.getElementById(`tab-run`),Xe=document.getElementById(`tab-files`),Ze=document.getElementById(`btn-run`),Qe=document.getElementById(`btn-stop`),$e=document.getElementById(`btn-save`),et=document.getElementById(`btn-open-dir`),tt=document.getElementById(`current-filename`),W=null,G=null,K=null,q=null,J=null,Y=!1;async function nt(){await it(),K=new Te(document.getElementById(`file-tree`)),await K.refresh(),q=new Ne(document.getElementById(`asset-panel`)),J=new Fe(document.getElementById(`console-panel`)),window.addEventListener(`message`,e=>{e.data?.__mce__&&J.log(e.data.level,e.data.msg)}),document.getElementById(`btn-console`).addEventListener(`click`,()=>{J.toggle()}),document.getElementById(`stab-files`).addEventListener(`click`,()=>ut(`files`)),document.getElementById(`stab-assets`).addEventListener(`click`,()=>ut(`assets`)),document.getElementById(`sidebar-backdrop`).addEventListener(`click`,()=>lt());let e=await D();e.length&&X(e[0]),j(N.FILE_OPEN,e=>X(e)),j(N.FILE_CREATE,()=>{K?.refresh(),q?.refresh()}),j(N.FILE_DELETE,e=>{G===e&&(G=null),K?.refresh(),q?.refresh()}),Je.addEventListener(`click`,()=>Q(`editor`)),Ye.addEventListener(`click`,()=>Q(`run`)),Xe.addEventListener(`click`,()=>lt()),Ze.addEventListener(`click`,at),Qe.addEventListener(`click`,()=>{ze(U),$(`Zatrzymano`,`info`)}),$e.addEventListener(`click`,Z),et.addEventListener(`click`,dt),document.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.key===`s`&&(e.preventDefault(),Z())}),document.addEventListener(`paste`,e=>{if(!W)return;let t=e.clipboardData?.getData(`text/plain`);t&&(W.hasTextFocus()||(e.preventDefault(),W.focus(),W.executeEdits(`paste`,[{range:W.getSelection(),text:t}])))}),document.getElementById(`btn-paste`)?.addEventListener(`click`,async()=>{try{let e=await navigator.clipboard.readText();if(!e||!W)return;W.focus(),W.executeEdits(`paste`,[{range:W.getSelection(),text:e}])}catch{$(`Brak dostępu do schowka`,`error`)}}),window.visualViewport&&(window.visualViewport.addEventListener(`resize`,rt),window.visualViewport.addEventListener(`scroll`,rt)),Q(`editor`)}function rt(){let e=window.visualViewport,t=document.getElementById(`app`);t.style.height=`${e.height}px`,t.style.transform=`translateY(${e.offsetTop}px)`,requestAnimationFrame(()=>W?.layout())}async function it(){(await D()).length>0||await T(`main.ts`,`import Phaser from 'phaser'
...
`)}async function X(e){G&&W&&await Z();let t=await w(e);if(t===null)return;G=e,tt.textContent=e,K?.setActive(e);let r=te(e);if(!W)W=ee(Ge,t,r);else{let i=c.parse(`file:///${e}`),a=n.getModel(i),o=a??n.createModel(t,r,i);a&&n.setModelLanguage(o,r),W.setModel(o)}Q(`editor`)}async function Z(){if(!G||!W)return;let e=W.getValue();await T(G,e),$(`Zapisano: ${G}`,`ok`)}async function at(){await Z(),J?.clear(),$(`Kompilowanie…`,`info`),Q(`run`);let e=await D(),t={};for(let n of e){let e=await w(n);e!==null&&(t[n]=e)}let n=G?.includes(`/`)?G.split(`/`).slice(0,-1).join(`/`)+`/`:``,r=n?`${n}index.html`:`index.html`,i=t[r];if(i){let e=ct(i,t,n),a=st(`HTML`,r,e.length);J?.setBuildInfo(a),$(`Uruchomiono ▶ (HTML)`,`ok`),Re({html:e,container:U});return}let a=G??e[0],o=performance.now(),s=await He(t,a);if(s.errors?.length){$(`Błąd: `+s.errors[0],`error`),J?.log(`error`,`✖ Błąd kompilacji: `+s.errors[0]),Q(`editor`);return}let c=Math.round(performance.now()-o),l=st(`ESM`,a,s.code.length,c);J?.setBuildInfo(l),$(`Uruchomiono ▶`,`ok`),Re({code:s.code,container:U})}var ot=`0.3.0`;function st(e,t,n,r){let i=(n/1024).toFixed(1),a=new Date,o=`${a.getHours().toString().padStart(2,`0`)}:${a.getMinutes().toString().padStart(2,`0`)}:${a.getSeconds().toString().padStart(2,`0`)}`;return[`▶ Build v${ot}`,`[${e}]`,t,`${i} KB`,r===void 0?``:`${r}ms`,`@ ${o}`].filter(Boolean).join(`  `)}function ct(e,t,n){return e.replace(/<script\s+src="([^"]+)"\s*><\/script>/gi,(e,r)=>{let i=t[r.startsWith(`./`)?n+r.slice(2):n+r]??t[r];return i===void 0?e:`<script>\n${i}\n<\/script>`})}function Q(e){Ge.style.display=e===`editor`?`flex`:`none`,Ke.style.display=e===`run`?`flex`:`none`,Je.classList.toggle(`active`,e===`editor`),Ye.classList.toggle(`active`,e===`run`),e===`editor`&&requestAnimationFrame(()=>W?.layout())}function lt(){Y=!Y,We.classList.toggle(`open`,Y),Xe.classList.toggle(`active`,Y),Ue.classList.toggle(`sidebar-open`,Y)}function ut(e){if(document.getElementById(`file-tree`).style.display=e===`files`?``:`none`,document.getElementById(`asset-panel`).style.display=e===`assets`?``:`none`,document.getElementById(`stab-files`).classList.toggle(`active`,e===`files`),document.getElementById(`stab-assets`).classList.toggle(`active`,e===`assets`),e===`assets`){let e=G?.includes(`/`)?G.split(`/`).slice(0,-1).join(`/`):``;q?.setFolder(e)}}function $(e,t){qe.textContent=e,qe.className=`status status-${t}`}async function dt(){try{let{openNativeFolder:e}=await o(async()=>{let{openNativeFolder:e}=await Promise.resolve().then(()=>be);return{openNativeFolder:e}},void 0);await e(),await K?.refresh(),$(`Folder otwarty`,`ok`)}catch(e){$(e.message??`Błąd`,`error`)}}nt();