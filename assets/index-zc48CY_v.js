...
<\/script>`,Ie=JSON.stringify({imports:{phaser:`https://esm.sh/phaser@3.88.2`,"pixi.js":`https://esm.sh/pixi.js@8`,"matter-js":`https://esm.sh/matter-js`}});function z({code:e,html:t,container:n}){B(n);let r=document.createElement(`iframe`);r.setAttribute(`sandbox`,`allow-scripts allow-same-origin`),r.style.cssText=`width:100%;height:100%;border:none;background:#000;flex:1`,t?r.srcdoc=t.includes(`<head>`)?t.replace(`<head>`,`<head>`+R):R+t:r.srcdoc=`<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  ...
  ${R}
  <script type="importmap">${Ie}<\/script>
</head>
<body>
...
</html>`,n.appendChild(r),L=r}function B(e){L&&=(L.remove(),null),e.querySelectorAll(`iframe`).forEach(e=>e.remove())}function Le(e){return new Worker(`/MobileCodeEditor/assets/compiler.worker-CKmcKM0R.js`,{type:`module`,name:e?.name})}var V=new Le,H=new Map;V.onmessage=e=>{let{id:t,code:n,errors:r}=e.data;H.get(t)?.({code:n,errors:r}),H.delete(t)};function Re(e,t){return new Promise(n=>{let r=Math.random().toString(36).slice(2);H.set(r,n),V.postMessage({id:r,files:e,entryPoint:t})})}var ze=document.getElementById(`app`),Be=document.getElementById(`sidebar`),Ve=document.getElementById(`editor-container`),He=document.getElementById(`run-container`),U=document.getElementById(`run-iframe-area`),Ue=document.getElementById(`status`),We=document.getElementById(`tab-editor`),Ge=document.getElementById(`tab-run`),Ke=document.getElementById(`tab-files`),qe=document.getElementById(`btn-run`),Je=document.getElementById(`btn-stop`),Ye=document.getElementById(`btn-save`),Xe=document.getElementById(`btn-open-dir`),Ze=document.getElementById(`current-filename`),W=null,G=null,K=null,q=null,J=null,Y=!1;async function Qe(){await et(),K=new Te(document.getElementById(`file-tree`)),await K.refresh(),q=new Ne(document.getElementById(`asset-panel`)),J=new Pe(document.getElementById(`console-panel`)),window.addEventListener(`message`,e=>{e.data?.__mce__&&J.log(e.data.level,e.data.msg)}),document.getElementById(`btn-console`).addEventListener(`click`,()=>{J.toggle()}),document.getElementById(`stab-files`).addEventListener(`click`,()=>ot(`files`)),document.getElementById(`stab-assets`).addEventListener(`click`,()=>ot(`assets`)),document.getElementById(`sidebar-backdrop`).addEventListener(`click`,()=>at());let e=await D();e.length&&X(e[0]),j(N.FILE_OPEN,e=>X(e)),j(N.FILE_CREATE,()=>{K?.refresh(),q?.refresh()}),j(N.FILE_DELETE,e=>{G===e&&(G=null),K?.refresh(),q?.refresh()}),We.addEventListener(`click`,()=>Q(`editor`)),Ge.addEventListener(`click`,()=>Q(`run`)),Ke.addEventListener(`click`,()=>at()),qe.addEventListener(`click`,tt),Je.addEventListener(`click`,()=>{B(U),$(`Zatrzymano`,`info`)}),Ye.addEventListener(`click`,Z),Xe.addEventListener(`click`,st),document.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.key===`s`&&(e.preventDefault(),Z())}),document.addEventListener(`paste`,e=>{if(!W)return;let t=e.clipboardData?.getData(`text/plain`);t&&(W.hasTextFocus()||(e.preventDefault(),W.focus(),W.executeEdits(`paste`,[{range:W.getSelection(),text:t}])))}),document.getElementById(`btn-paste`)?.addEventListener(`click`,async()=>{try{let e=await navigator.clipboard.readText();if(!e||!W)return;W.focus(),W.executeEdits(`paste`,[{range:W.getSelection(),text:e}])}catch{$(`Brak dostępu do schowka`,`error`)}}),window.visualViewport&&(window.visualViewport.addEventListener(`resize`,$e),window.visualViewport.addEventListener(`scroll`,$e)),Q(`editor`)}function $e(){let e=window.visualViewport,t=document.getElementById(`app`);t.style.height=`${e.height}px`,t.style.transform=`translateY(${e.offsetTop}px)`,requestAnimationFrame(()=>W?.layout())}async function et(){(await D()).length>0||await T(`main.ts`,`import Phaser from 'phaser'
...
`)}async function X(e){G&&W&&await Z();let t=await w(e);if(t===null)return;G=e,Ze.textContent=e,K?.setActive(e);let r=te(e);if(!W)W=ee(Ve,t,r);else{let i=c.parse(`file:///${e}`),a=n.getModel(i),o=a??n.createModel(t,r,i);a&&n.setModelLanguage(o,r),W.setModel(o)}Q(`editor`)}async function Z(){if(!G||!W)return;let e=W.getValue();await T(G,e),$(`Zapisano: ${G}`,`ok`)}async function tt(){await Z(),J?.clear(),$(`Kompilowanie…`,`info`),Q(`run`);let e=await D(),t={};for(let n of e){let e=await w(n);e!==null&&(t[n]=e)}let n=G?.includes(`/`)?G.split(`/`).slice(0,-1).join(`/`)+`/`:``,r=n?`${n}index.html`:`index.html`,i=t[r];if(i){let e=it(i,t,n),a=rt(`HTML`,r,e.length);J?.setBuildInfo(a),$(`Uruchomiono ▶ (HTML)`,`ok`),z({html:e,container:U});return}let a=G??e[0],o=performance.now(),s=await Re(t,a);if(s.errors?.length){$(`Błąd: `+s.errors[0],`error`),J?.log(`error`,`✖ Błąd kompilacji: `+s.errors[0]),Q(`editor`);return}let c=Math.round(performance.now()-o),l=rt(`ESM`,a,s.code.length,c);J?.setBuildInfo(l),$(`Uruchomiono ▶`,`ok`),z({code:s.code,container:U})}var nt=`0.3.0`;function rt(e,t,n,r){let i=(n/1024).toFixed(1),a=new Date,o=`${a.getHours().toString().padStart(2,`0`)}:${a.getMinutes().toString().padStart(2,`0`)}:${a.getSeconds().toString().padStart(2,`0`)}`;return[`▶ Build v${nt}`,`[${e}]`,t,`${i} KB`,r===void 0?``:`${r}ms`,`@ ${o}`].filter(Boolean).join(`  `)}function it(e,t,n){return e.replace(/<script\s+src="([^"]+)"\s*><\/script>/gi,(e,r)=>{let i=t[r.startsWith(`./`)?n+r.slice(2):n+r]??t[r];return i===void 0?e:`<script>\n${i}\n<\/script>`})}function Q(e){Ve.style.display=e===`editor`?`flex`:`none`,He.style.display=e===`run`?`flex`:`none`,We.classList.toggle(`active`,e===`editor`),Ge.classList.toggle(`active`,e===`run`),e===`editor`&&requestAnimationFrame(()=>W?.layout())}function at(){Y=!Y,Be.classList.toggle(`open`,Y),Ke.classList.toggle(`active`,Y),ze.classList.toggle(`sidebar-open`,Y)}function ot(e){if(document.getElementById(`file-tree`).style.display=e===`files`?``:`none`,document.getElementById(`asset-panel`).style.display=e===`assets`?``:`none`,document.getElementById(`stab-files`).classList.toggle(`active`,e===`files`),document.getElementById(`stab-assets`).classList.toggle(`active`,e===`assets`),e===`assets`){let e=G?.includes(`/`)?G.split(`/`).slice(0,-1).join(`/`):``;q?.setFolder(e)}}function $(e,t){Ue.textContent=e,Ue.className=`status status-${t}`}async function st(){try{let{openNativeFolder:e}=await o(async()=>{let{openNativeFolder:e}=await Promise.resolve().then(()=>be);return{openNativeFolder:e}},void 0);await e(),await K?.refresh(),$(`Folder otwarty`,`ok`)}catch(e){$(e.message??`Błąd`,`error`)}}Qe();